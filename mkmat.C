...
//int xmlread_dir(int run=10){
int mkmat_openrun(int run=10){
  char file[500];// t1....
    //  V, addwith
  //   0=telescopes;  1=t1  2=V
    ...
    addr[1]="location";
    addr[0]="dir";
    getattrlevel=1; // THIS MAY DESTROY ALL THE REST
    xmlreadfile_init("mkmat.xml");
   DisplayTele(xml, mainnode, 0, "dir" );
   printf("GO ON with %s\n",  output );
   //
   //  ls -1 run*root | grep -e 'run[0-9]\+_'
   //
   sprintf( file, "%s/run%d.root", output, run );
   printf("i ... going to open %s\n", file);
    xmlreadfile_fini();
   TFile *f=new TFile(file);
   f->ls();
   getattrlevel=3; // THIS MAY RESTORE ALL THE REST
...
void DisplayTele(TXMLEngine* xml, XMLNodePointer_t node, Int_t level, const char* search )
{
  ...
  if (leveloffset>=0){ // there exists search @ eveloffset
    if ( tel.CompareTo( search  )==0){
